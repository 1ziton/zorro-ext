...
import { Column, PrimeTemplate, PrimeNGSharedModule } from '../common/shared';
    ...
    template: `
        <div #container [ngStyle]="style" [class]="styleClass" 
            ...
            <div class="ui-table-loading-content" *ngIf="loading">
                <i class="anticon anticon-spin anticon-loading" style="color:#EE7F25;font-size:18px;"></i>
            </div>
...
export class Table implements OnInit, AfterContentInit, OnDestroy {
    ...
    previousEditedField: string;

    previousEditedRowIndex: number;

    editingRowIndex: number;

    documentMouseClickListener: any;
    //
    ...
    bindDocumentEvents() {
        this.zone.runOutsideAngular(() => {
            this.documentMouseClickListener = this.onDocumentMouseClick.bind(this);
            document.addEventListener('click', this.documentMouseClickListener);
        });
    }

    unbindDocumentEvents() {

        if (this.documentMouseClickListener) {
            document.removeEventListener('click', this.documentMouseClickListener);
            this.documentMouseClickListener = null;
        }
    }

    onDocumentMouseClick(event: Event) {
        // event.preventDefault();
        event.stopPropagation();
        if (event.target && event.target['nodeName'] && this.editingCell) {
            let nodeName = event.target['nodeName'];
            if (nodeName === 'DIV' && this.editingCell) {
                this.zone.run(() => {
                    //保存取消编辑
                    this.domHandler.removeClass(this.editingCell, 'ui-editing-cell');
                    this.onEditComplete.emit({ field: this.previousEditedField, data: this.previousEditedData, rowIndex: this.previousEditedRowIndex });
...
export class EditableColumn implements AfterViewInit {
    ...
    @Input("pEditableRowIndex") index: any;
    ...
    onClick(event: MouseEvent) {
        if (this.isEnabled()) {
            this.dt.editingRowIndex = this.index;
            ...
            if (this.dt.editingCell) {
                ...
                if (this.dt.editingCell !== this.el.nativeElement) {
                    ...
                    this.dt.onEditComplete.emit({ field: this.dt.previousEditedField, data: this.dt.previousEditedData, rowIndex: this.dt.previousEditedRowIndex });
                    setTimeout(() => {
                        this.openCell();
                    }, 0);
                }
    ...
    openCell() {
        this.dt.previousEditedData = JSON.parse(JSON.stringify(this.data));
        this.dt.previousEditedField = this.field;
        this.dt.previousEditedRowIndex = this.index;
        this.dt.editingCell = this.el.nativeElement;
        this.domHandler.addClass(this.el.nativeElement, 'ui-editing-cell');
        this.dt.onEditInit.emit({ field: this.field, data: this.data, rowIndex: this.index });
        this.zone.runOutsideAngular(() => {
            setTimeout(() => {
        ...
        if (this.isEnabled()) {
            ...
            if (event.keyCode == 13) {
                if (this.isValid()) {
                    this.domHandler.removeClass(this.dt.editingCell, 'ui-editing-cell');
                    this.dt.editingCell = null;
                    this.dt.onEditComplete.emit({ field: this.field, data: this.data, rowIndex: this.index });
                }
            ...
            //escape
                ...
                if (this.isValid()) {
                    this.domHandler.removeClass(this.dt.editingCell, 'ui-editing-cell');
                    this.dt.editingCell = null;
                    this.dt.onEditCancel.emit({ field: this.field, data: this.data, rowIndex: this.index });
                }
            ...
            //tab
                ...
                // giscafer add 2018年3月29日10:21:38
                if (this.isValid()) {
                    // this.domHandler.removeClass(this.dt.editingCell, 'ui-editing-cell');
                    // this.dt.editingCell = null;
                    this.dt.onEditComplete.emit({ field: this.field, data: this.data, rowIndex: this.index });
...
@NgModule({
    ...
    exports: [Table, PrimeNGSharedModule, SortableColumn, SelectableRow, RowToggler, ContextMenuRow, ResizableColumn, ReorderableColumn, EditableColumn, CellEditor, SortIcon, TableRadioButton, TableCheckbox, TableHeaderCheckbox, ReorderableRowHandle, ReorderableRow, SelectableRowDblClick],
